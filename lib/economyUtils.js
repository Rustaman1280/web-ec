...
export const awardBadge = async (uid, badgeId, badgeName, badgeIcon, rewardPoints = 0, rewardExp = 0) => {
  if(!database) return false;
  
  const badgeRef = ref(database, `users/${uid}/badges/${badgeId}`);
  const badgeSnap = await get(badgeRef);
  
  // 1. Award Badge (Overwrite allowed for upgrading or just ensuring it's there)
  await set(badgeRef, {
    name: badgeName,
    icon: badgeIcon,
    earnedAt: Date.now()
  ...
  // 2. Give Rewards if first time earning (Check snap)
  if (!badgeSnap.exists() && (rewardPoints > 0 || rewardExp > 0)) {
    const userRef = ref(database, `users/${uid}`);
    const userSnap = await get(userRef);
    if(userSnap.exists()) {
       const uData = userSnap.val();
       await update(userRef, {
          points: (uData.points || 0) + rewardPoints,
          exp: (uData.exp || 0) + rewardExp
       });
       await recordTransaction(uid, 'badge_reward', `Earned Badge: ${badgeName}`, rewardPoints, rewardExp);
    }
  }
...
export const getDailyStreakData = async (uid) => {
...
// Add Daily Claim Logic (Replaces the old recordAttendance)
export const recordDailyClaim = async (uid) => {
  if(!database) return false;
  
  const { streakCount, checkedInToday } = await getDailyStreakData(uid);
  ...
  if(userSnap.exists()) {
     ...
     // Log to Ledger
     await recordTransaction(uid, 'daily_claim', `Daily Claim (Day ${newStreak})`, rewardAmt, rewardAmt);
     return { success: true, rewardAmt, newStreak };
  }
  return false;
};

// Admin: Mark Attendance (Wednesday Club Meeting)
export const adminMarkAttendance = async (uid, dateStr, isPresent) => {
   if(!database) return false;

   // Strict Restriction: Only Wednesdays allowed (3 = Wednesday)
   const day = new Date(dateStr).getDay();
   if (day !== 3) {
      throw new Error("Attendance can only be recorded for Wednesdays!");
   }
   ...
   if(snap.exists()) return true; // Already present

   await set(attRef, { timestamp: Date.now() });
   
   // Reward for school attendance (e.g., 20 Pts + 20 EXP)
   const rewardPts = 20;
   const rewardExp = 20;
   ...
   if(userSnap.exists()) {
      ...
      await recordTransaction(uid, 'school_attendance', `Ekskul Attendance (${dateStr})`, rewardPts, rewardExp);
   }
   return true;
};
