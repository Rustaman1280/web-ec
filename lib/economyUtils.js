...
export const recordAttendance = async (uid) => {
  ...
  const attRef = ref(database, `attendance/${today}/${uid}`);
  ...
  // Reward player (e.g. +5 EXP for logging in)
  const userRef = ref(database, `users/${uid}`);
  const userSnap = await get(userRef);
  if(userSnap.exists()) {
     const user = userSnap.val();
     await update(userRef, { exp: (user.exp || 0) + 5 });
     // Log to Ledger
     await recordTransaction(uid, 'attendance', `Daily Check-In Bonus`, 0, 5);
  }
  return true;
};
