...
export const getLiveQuizById = async (id) => {
  if(!database) return null;
  const snap = await get(ref(database, `live_quizzes/${id}`));
  if (snap.exists()) return { id, ...snap.val() };
  return null;
};

export const updateLiveQuizBank = async (id, title, description, color, icon, sessionPoints, sessionExp, questions, bgTheme = 'default', bgMusic = 'none') => {
  if(!database) return;
  await update(ref(database, `live_quizzes/${id}`), { title, description, color, icon, sessionPoints: parseInt(sessionPoints)||0, sessionExp: parseInt(sessionExp)||0, questions, bgTheme, bgMusic, updatedAt: Date.now() });
};
...
export const submitAnswer = async (pin, playerId, answerIndex, timeTakenMs, isCorrect) => {
  ...
  if (snapshot.exists()) {
    ...
    const historyEntry = {
        isCorrect: isCorrect,
        pointsEarned: leaderboardScoreEarned,
        timeTakenMs: timeTakenMs
    };
    const newAnswersHistory = pData.answersHistory || [];
    newAnswersHistory[session.currentQuestionIndex] = historyEntry;
    ...
    await update(playerRef, {
      ...
      correctCount: (pData.correctCount || 0) + (isCorrect ? 1 : 0),
      wrongCount: (pData.wrongCount || 0) + (!isCorrect ? 1 : 0),
...
export const saveQuizHistory = async (pin, session) => {
  ...
  await set(historyRef, {
    ...
    topPlayers: sorted.slice(0, 3).map((p, i) => ({ ...p, rank: i + 1, correctCount: participants[p.id]?.correctCount || 0, wrongCount: participants[p.id]?.wrongCount || 0 })),
    participants: Object.fromEntries(
      sorted.map((p, i) => [p.id, { 
         name: p.name, 
         score: p.score, 
         finalRank: i + 1,
         correctCount: participants[p.id]?.correctCount || 0,
         wrongCount: participants[p.id]?.wrongCount || 0
      }])
    )
  });
};
  ...
  if (snap.exists()) {
    ...
    return Object.keys(data)
      ...
      .map(h => ({
        ...
        myCorrectCount: h.participants[uid].correctCount || 0,
        myWrongCount: h.participants[uid].wrongCount || 0
      }))
...
// ==========================================
// SPEECHY GAME FUNCTIONS
// ==========================================

export const getAllSpeechyBanks = async () => {
  if (!database) return [];
  const snap = await get(ref(database, 'speechy_banks'));
  if (snap.exists()) {
     const data = snap.val();
     return Object.keys(data).map(k => ({ id: k, ...data[k] }));
  }
...
export const deleteSpeechyBank = async (id) => {
  if (!database) return;
  await remove(ref(database, `speechy_banks/${id}`));
};

export const saveSpeechyBank = async (id, data) => {
  if (!database) return;
  if (id) {
     await update(ref(database, `speechy_banks/${id}`), data);
     return id;
  } else {
     const newRef = push(ref(database, 'speechy_banks'));
     await set(newRef, { ...data, createdAt: Date.now() });
     return newRef.key;
  }
};

export const getSpeechySubmissions = async (bankId) => {
  if (!database) return {};
  const snap = await get(ref(database, `speechy_submissions/${bankId}`));
  return snap.exists() ? snap.val() : {};
};

export const submitSpeechyAsync = async (bankId, uid, profileName, accuracy, timeTakenMs, points, exp, title, transcript) => {
  if (!database) return;
  const subRef = ref(database, `speechy_submissions/${bankId}/${uid}`);
  ...
  const dataToSave = {
     ...
     accuracy,
     timeTakenMs,
     bestAccuracy: isFirstTime ? accuracy : Math.max(accuracy, snap.val().bestAccuracy || 0),
     lastPlayedAt: Date.now(),
     transcript
  };

  await update(ref(database, `speechy_submissions/${bankId}`), {
      [uid]: dataToSave
  });
  ...
  if (isFirstTime) {
     ...
     if (userSnap.exists()) {
         ...
         await recordTransaction(uid, 'earn_speechy', `Completed Speechy: ${title}`, points, exp);
     }
  }
  return { isFirstTime };
};
