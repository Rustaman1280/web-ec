...
export const createLiveQuizBank = async (title, description, color, icon, sessionPoints, sessionExp, questions) => {
  if(!database) return null;
  const qRef = push(ref(database, 'live_quizzes'));
  await set(qRef, { title, description, color, icon, sessionPoints: parseInt(sessionPoints)||0, sessionExp: parseInt(sessionExp)||0, questions, createdAt: Date.now() });
  return qRef.key;
};
...
// Create a new room 
export const createQuizSession = async (quizId, sessionPoints, sessionExp, questions) => {
  if(!database) return null;
...
export const submitAnswer = async (pin, playerId, answerIndex, timeTakenMs, isCorrect) => {
  ...
  const leaderboardScoreEarned = basePoints + timeBonus; // 500 to 1000
  
  // Real Economy Payout per question
  const expEarned = isCorrect ? (parseInt(currentQ.rewardExp) || 0) : 0;
  const coinsEarned = isCorrect ? (parseInt(currentQ.rewardPoints) || 0) : 0;
  ...
  if (expEarned > 0 || coinsEarned > 0) {
    ...
    if(userSnap.exists()) {
        ...
        await recordTransaction(playerId, 'earn_quiz', `Correct Answer: ${session.title || 'Live Quiz'}`, coinsEarned, expEarned);
    }
  }
};
...
export const awardSessionRewards = async (pin) => {
  ...
  if (sExp === 0 && sPts === 0) return;
  
  const participants = session.participants || {};
  for (const uid of Object.keys(participants)) {
     const p = participants[uid];
     // Give session rewards to everyone who entered and answered at least something or just joined
