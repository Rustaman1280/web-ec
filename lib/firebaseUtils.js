...
export const createSpeechySession = async (bankId, sessionPoints, sessionExp, targetText, bgTheme, bgMusic, title) => {
  if (!database) return null;
  const pinObj = Array.from({ length: 6 }, () => Math.floor(Math.random() * 10)).join('');
  const pin = pinObj;
